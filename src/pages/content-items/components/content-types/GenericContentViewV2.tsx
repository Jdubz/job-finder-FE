...
export const GenericContentViewV2: React.FC<GenericContentViewProps> = ({ item }) => {
  ...
  const typeKey = item.type === "skill-group" ? "skillGroup" : 
                  item.type === "text-section" ? "textSection" :
                  item.type === "profile-section" ? "profileSection" :
                  item.type === "timeline-event" ? "timelineEvent" : item.type
          ...
          <div className="space-y-4">
            ...
            {(getProperty("startDate") || getProperty("endDate")) && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>
                  {formatDate(getProperty("startDate"))} - {formatDate(getProperty("endDate")) || "Present"}
                </span>
          ...
          <div className="space-y-4">
            ...
            {getProperty("content") && (
              <div className="prose prose-sm max-w-none">
                <p className="text-muted-foreground whitespace-pre-wrap">{getProperty("content")}</p>
              </div>
            )}
          </div>
        )
      ...
      default:
        return (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <IconComponent className="h-5 w-5 text-primary" />
              <h3 className="text-xl font-semibold">{item.type.replace("-", " ").replace(/\b\w/g, l => l.toUpperCase())}</h3>
            </div>
